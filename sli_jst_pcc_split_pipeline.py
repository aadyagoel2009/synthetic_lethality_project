"""
JST-PCC type-split: one merged ranking with feature-type-specific context.

Expression features (_exp suffix):
  S_exp = C_sclc * (1 + lambda * max(0, zscore(transfer_AGOP)))

Mutation features (all other feature columns):
  S_mut = AGOP_pan * C_pan          (thesis SL-RFM: g_tilde ⊙ c on pan lines)
...
Baselines: pan_fi, hybrid, sclc_pcc_only.

Run: python sli_jst_pcc_split_pipeline.py
"""
...
RESULTS_DIR = "sli_jst_pcc_split_results"
...
# ---------------------------------------------------------------------------
def is_exp_feature(feature: str) -> bool:
    return str(feature).split("_")[-1] == "exp"


def split_feature_index(index: pd.Index) -> Tuple[List[str], List[str]]:
    exp_idx = [str(f) for f in index if is_exp_feature(f)]
    mut_idx = [str(f) for f in index if not is_exp_feature(f)]
    return exp_idx, mut_idx
...
# ---------------------------------------------------------------------------
# P_pan, pan FI, JST structure, type-split merge
# ---------------------------------------------------------------------------
...
def compute_pan_fi(
    ...
    """Thesis SL-RFM on pan: AGOP(alpha_0) * process_pcc(pan PCC)."""
...
def compute_exp_jst_pcc_scores(
    pcc_sclc: pd.DataFrame,
    structure_df: pd.DataFrame,
    lam: float,
) -> pd.DataFrame:
    """Expression channel: C_sclc * (1 + lambda * max(0, z(structure)))."""
...
def merge_type_split_scores(
    exp_channel: pd.DataFrame,
    mut_channel: pd.DataFrame,
) -> pd.DataFrame:
    """
    Per KO column: percentile-rank exp scores among exp features and mut scores
    among mut features; assign each feature its within-type percentile.
    Higher percentile = better (ranked higher in final list).
    """
    ...
    exp_idx, mut_idx = split_feature_index(common_idx)

    exp_ch = exp_channel.loc[common_idx, common_cols].astype(float)
    mut_ch = mut_channel.loc[common_idx, common_cols].astype(float)
    out = pd.DataFrame(index=common_idx, columns=common_cols, dtype=float)

    for col in common_cols:
        if exp_idx:
            s = exp_ch.loc[exp_idx, col]
            out.loc[exp_idx, col] = s.rank(pct=True, method="average")
        if mut_idx:
            s = mut_ch.loc[mut_idx, col]
            out.loc[mut_idx, col] = s.rank(pct=True, method="average")

    return out


def method_tag(gamma: float, lam: float) -> str:
    return f"split_g{gamma:g}_lam{lam:g}"
...
def rank_all_features_for_one_ko(scores: pd.Series, ko_gene: str) -> pd.DataFrame:
    ...
    df["feature_type"] = np.where(
        df["feature"].astype(str).str.endswith("_exp"), "exp", "mut"
    )
        ...
        rows.append(
            {
                ...
                "top1_feature_type": "exp" if str(s.index[0]).endswith("_exp") else "mut",
                "top1_score": top1,
...
def main() -> None:
    ...
    print("Type-split: exp=SCLC JST-PCC, mut=pan SL-RFM, merge=within-type percentiles")
    ...
    print("\n=== Mutation channel (pan SL-RFM, fixed) ===")
    mut_channel = fi_pan.reindex(
        index=pcc_sclc_eval.index, columns=eval_kos
    ).fillna(0)
    ...
    for gamma in JST_GAMMA_SWEEP:
        print(f"\n=== JST structure for exp channel (gamma={gamma}) ===")
    ...
    print("\n=== Primary: type-split merged ranking ===")
    for gamma in JST_GAMMA_SWEEP:
        for lam in JST_PCC_LAMBDA_SWEEP:
            tag = method_tag(gamma, lam)
            exp_channel = compute_exp_jst_pcc_scores(
                pcc_sclc_eval, structure_by_gamma[gamma], lam
            )
            merged = merge_type_split_scores(exp_channel, mut_channel)
            fi_by_tag[tag] = merged
            eval_rows.append(evaluate_panel(merged, tag))
    ...
    split_rows = rank_summary_df[rank_summary_df["method"].str.startswith("split_")]
    if len(split_rows):
        print(f"\nBest type-split: {split_rows.iloc[0]['method']}")
    sclc_row = rank_summary_df[rank_summary_df["method"] == "sclc_pcc_only"]
    pan_row = rank_summary_df[rank_summary_df["method"] == "pan_fi"]
    if len(sclc_row):
        print(f"  sclc_pcc_only mean rank: {sclc_row.iloc[0]['mean_panel_rank']:.1f}")
    if len(pan_row):
        print(f"  pan_fi mean rank:        {pan_row.iloc[0]['mean_panel_rank']:.1f}")
    ...
    if fi_default is not None:
        build_top_pair_per_ko(fi_default).to_csv(
            os.path.join(RESULTS_DIR, "top_pairs_split_default.csv"), index=False
        )
        fi_default.to_pickle(os.path.join(RESULTS_DIR, f"fi_{default_tag}.pkl"))
        mut_channel.to_pickle(os.path.join(RESULTS_DIR, "mut_channel_pan_fi.pkl"))

    xlsx = os.path.join(RESULTS_DIR, "sli_jst_pcc_split_summary.xlsx")
