...
JST_PCC_LAMBDA_SWEEP: List[float] = [
    0.0,
    0.01,
    0.05,
    0.1,
...
def validate_sclc_in_pan(
    ...
    if not np.allclose(
        ...
        print(
            f"Warning: SCLC .hkl rows differ from pan "
            f"(max diff={diff:.6g}); using pan."
        )
...
# ---------------------------------------------------------------------------
# Manual mutation override helper functions
# ---------------------------------------------------------------------------
def normalize_cell_line_name(x: str) -> str:
    """
...
def find_model_id_for_cell_line(
    model_path: str,
    cell_line_name: str,
) -> str:
    """
    Find the DepMap ModelID for a cell line name like NCI-H1048.
    """
    meta = pd.read_csv(model_path)

    if "ModelID" not in meta.columns:
        raise ValueError("Model.csv does not have a ModelID column.")
    ...
    candidate_name_cols = [c for c in candidate_name_cols if c in meta.columns]

    target = normalize_cell_line_name(cell_line_name)
    ...
    for col in candidate_name_cols:
        normalized_col = (
            ...
            .fillna("")
            .astype(str)
            .apply(normalize_cell_line_name)
        )

        hits = meta.loc[
            normalized_col == target,
            "ModelID",
        ].astype(str).tolist()
        ...
        if hits:
            if len(hits) > 1:
                print(
                    f"Warning: multiple matches for {cell_line_name}: {hits}. "
                    f"Using first."
                )
            return hits[0]

    raise ValueError(
        f"Could not find ModelID for cell line {cell_line_name}. "
        f"Checked columns: {candidate_name_cols}"
    )


def force_mutation_present(
    embedding_df: pd.DataFrame,
    model_id: str,
    gene: str,
) -> pd.DataFrame:
    """
    Force a mutation feature to be present for one cell line.

    In this pipeline:
      CREBBP     = mutation feature
      CREBBP_exp = expression feature

    So this function sets CREBBP = 1.0 for the selected model.
    """
    ...
    if mutation_col not in out.columns:
        print(f"Warning: {mutation_col} column does not exist. Creating it.")
        out[mutation_col] = 0.0

    if model_id not in out.index:
        raise ValueError(
            f"ModelID {model_id} not found in embedding index. "
            f"Cannot force {gene} mutation for this embedding."
        )

    before_value = out.loc[model_id, mutation_col]
    out.loc[model_id, mutation_col] = 1.0
    after_value = out.loc[model_id, mutation_col]

    print(
        f"Forced {gene} mutation present for {model_id}: "
...
def build_kernel_matrix(
    ...
    dist = euclidean_distances_torch(
        X_t,
        X_t,
        M=P_t,
        squared=False,
...
def train_krr_joint(
    ...
    With gamma > 0:
      (K_w + (reg + gamma)I) alpha = Y_w + gamma * alpha_prior
    """
...
# PCC
# ---------------------------------------------------------------------------
def _embedding_with_exp_outlier_mask(
    ...
    z = (
        cell_embedding[exp_cols]
        - cell_embedding[exp_cols].mean(axis=0)
    ) / std_val
...
def get_pcc(
    ...
    return pd.DataFrame(
        pcc,
        index=norm_emb.columns,
        columns=norm_ge.columns,
    )
...
def compute_kmm_weights(
    ...
    sclc_idx = np.array(
        [i for i, row in enumerate(index) if row in sclc_ids_in_pan]
    )

    lung_idx = np.array(
        [i for i, row in enumerate(index) if row in lung_ids_in_pan]
    )

    if len(sclc_idx) == 0:
        raise ValueError("No SCLC lines in pan embedding for KMM target.")

    mu_target = compute_target_centroid(
        X,
        sclc_idx,
        lung_idx,
        sclc_frac,
    ...
    for _ in range(n_iter):
        ...
        adjust = np.exp(
            -0.5
            * (
                np.linalg.norm(X - mu_target, axis=1)
                / (sigma + 0.25 * scale)
            )
            ** 2
        )
    ...
    print(
        f"  KMM weights: sum={omega.sum():.1f}, "
        f"SCLC mean={omega[sclc_idx].mean():.3f} "
...
def load_or_compute_p_pan(
    ...
    p_pan = stabilize_metric_p(
        pd.Series(
            grads.detach().cpu().numpy().mean(axis=1),
...
def compute_pan_fi(
    ...
    if all(
        ...
        for s in ("_data.npy", "_index.npy", "_columns.npy")
    ):
    ...
    pcc = pcc.reindex(
        index=grads_df.index,
        columns=grads_df.columns,
    ).fillna(0)
...
def compute_jst_structure(
    ...
    """
    Pan transfer structure:
      AGOP(alpha*) with joint KRR + KMM weights + pan prior.
    """
...
def compute_jst_pcc_scores(
    ...
    """
    S = C_sclc * (1 + lambda * max(0, zscore(AGOP))).

    lambda=0 gives sclc_pcc_only.
    """
...
def build_hybrid_fi(
    ...
    return pd.DataFrame(
        {
            ko: hybrid_score_for_ko(fi_pan, pcc_sclc, ko)
...
# Evaluation
# ---------------------------------------------------------------------------
def rank_all_features_for_one_ko(
    scores: pd.Series,
    ko_gene: str,
) -> pd.DataFrame:
...
def evaluate_panel(
    ...
    for title, ko, partner in KNOWN_SLI_PANELS:
        ...
        rank, score, feat = get_partner_rank_and_score(
            col,
            ko,
            partner,
        )
...
def main() -> None:
    ...
    # -----------------------------------------------------------------------
    # Load embeddings WITHOUT normalization first.
    # We need to force the CREBBP mutation before row L2 normalization.
    # -----------------------------------------------------------------------
    pan_embedding = load_hkl_dataframe(PAN_EMBED_HKL, "pan")
    sclc_embedding = load_hkl_dataframe(SCLC_EMBED_HKL, "sclc")

    pan_embedding.index = pan_embedding.index.astype(str)
    sclc_embedding.index = sclc_embedding.index.astype(str)

    # -----------------------------------------------------------------------
    # Force CREBBP mutation present for NCI-H1048.
    # -----------------------------------------------------------------------
    nci_h1048_model_id = find_model_id_for_cell_line(
        model_path=MODEL_CSV_PATH,
        cell_line_name="NCI-H1048",
    )

    print(f"NCI-H1048 ModelID: {nci_h1048_model_id}")

    pan_embedding = force_mutation_present(
        embedding_df=pan_embedding,
        model_id=nci_h1048_model_id,
        gene="CREBBP",
    )

    sclc_embedding = force_mutation_present(
        embedding_df=sclc_embedding,
        model_id=nci_h1048_model_id,
        gene="CREBBP",
    )

    # -----------------------------------------------------------------------
    # Now normalize after the manual override.
    # -----------------------------------------------------------------------
    pan_embedding = ensure_row_l2_normalized(pan_embedding)
    sclc_embedding = ensure_row_l2_normalized(sclc_embedding)

    print(
        "After normalization, CREBBP value for NCI-H1048 in pan:",
        pan_embedding.loc[nci_h1048_model_id, "CREBBP"],
    )

    print(
        "After normalization, CREBBP value for NCI-H1048 in SCLC:",
        sclc_embedding.loc[nci_h1048_model_id, "CREBBP"],
    )
    ...
    for gamma in JST_GAMMA_SWEEP:
        for lam in JST_PCC_LAMBDA_SWEEP:
            ...
            fi = compute_jst_pcc_scores(
                pcc_sclc_eval,
                structure_by_gamma[gamma],
                lam,
            )
    ...
    rank_summary = [
        {
            "method": m,
            "mean_panel_rank": mean_panel_rank(eval_df, m),
        }
        for m in eval_df["method"].unique()
    ]

    rank_summary_df = pd.DataFrame(rank_summary).sort_values(
        "mean_panel_rank"
    )
    ...
    jst_rows = rank_summary_df[
        rank_summary_df["method"].str.startswith("jst_pcc_")
    ]

    if len(jst_rows):
        print(f"\nBest JST-PCC: {jst_rows.iloc[0]['method']}")

    sclc_row = rank_summary_df[
        rank_summary_df["method"] == "sclc_pcc_only"
    ]

    if len(sclc_row):
        print(
            f"  sclc_pcc_only mean rank: "
            f"{sclc_row.iloc[0]['mean_panel_rank']:.1f}"
        )
    ...
    best_csv = os.path.join(
        RESULTS_DIR,
        "best_method_per_panel.csv",
    )

    best_per_panel.to_csv(
        best_csv,
        index=False,
    )

    default_tag = method_tag(
        DEFAULT_JST_GAMMA,
        DEFAULT_JST_PCC_LAMBDA,
    )
    ...
    if fi_default is not None:
        build_top_pair_per_ko(fi_default).to_csv(
            os.path.join(
                RESULTS_DIR,
                "top_pairs_jst_pcc_default.csv",
            ),
            index=False,
        )

        fi_default.to_pickle(
            os.path.join(
                RESULTS_DIR,
                f"fi_{default_tag}.pkl",
            )
        )

    xlsx = os.path.join(
        RESULTS_DIR,
        "sli_jst_pcc_summary.xlsx",
    )

    with pd.ExcelWriter(xlsx, engine="openpyxl") as writer:
        eval_df.to_excel(
            writer,
            sheet_name="PanelRankComparison",
            index=False,
        )

        best_per_panel.to_excel(
            writer,
            sheet_name="BestMethodPerPanel",
            index=False,
        )

        rank_summary_df.to_excel(
            writer,
            sheet_name="MeanPanelRank",
            index=False,
        )
