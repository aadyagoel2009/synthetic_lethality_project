...
# Load embedding and CRISPR gene effects
# (paths based on what you used earlier)
embedding = hkl.load("embeddings/final_X_tcga_processed.hkl")
...
print("gene_effects_df:", gene_effects_df.shape)

vals = np.load("datasets/feature_importances_data.npy")
idx  = np.load("datasets/feature_importances_index.npy", allow_pickle=True)
cols = np.load("datasets/feature_importances_columns.npy", allow_pickle=True)

feature_importance_df = pd.DataFrame(vals, index=idx, columns=cols)
