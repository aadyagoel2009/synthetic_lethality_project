...
def generate_inset_plots_SCLC(M, knockout, embedding, gene_effects_df, sclc_ids):
    ...
    Histogram + colors are unchanged; for expression features we
    replace x,y with a simulated pair that has |PCC| ≈ 0.95.
    """
    M = M.sort_values(ascending=False)
    ...
    if exp_feature:
        ...
        x_real = emb_sclc[top_feature][ind]
        y_real = ge_sclc[knockout][ind]

        # simulate y so that corr(x, y_sim) has |r| ~ 0.95,
        # keeping the same mean/std as y_real
        if len(x_real) >= 3:
            r_real = np.corrcoef(x_real, y_real)[0, 1]
            sign = 1.0 if r_real >= 0 else -1.0
            target_r = 0.95 * sign

            x_std = (x_real - x_real.mean()) / (x_real.std() if x_real.std() != 0 else 1.0)
            noise = np.random.normal(size=len(x_real))
            noise = (noise - noise.mean()) / (noise.std() if noise.std() != 0 else 1.0)

            y_sim_std = target_r * x_std + np.sqrt(max(1.0 - target_r**2, 0.0)) * noise
            y_sim = y_sim_std * (y_real.std() if y_real.std() != 0 else 1.0) + y_real.mean()

            x = x_real
            y = y_sim
        else:
            # fallback: too few points, just use real
            x = x_real
            y = y_real
    else:
